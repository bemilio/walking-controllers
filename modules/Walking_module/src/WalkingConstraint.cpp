...
PositionConstraint::PositionConstraint(const int& jacobianCols)
{
    ...
    m_controllers.insert({"positional_pid", std::make_shared<LinearPID>()});
...
void GenericCartesianConstraint::evaluateBounds()
{
    iDynTree::toEigen(m_upperBound) = iDynTree::toEigen(m_desiredAcceleration)
        ...
        iDynTree::toEigen(m_generalizedBiasForces).inverse();
}
...
void CartesianConstraint::evaluateDesiredAcceleration()
{
    ...
    iDynTree::toEigen(m_desiredAcceleration).block(0, 3, 3, 1)
      ...
      m_areBoundsEvaluated(false)
{
    ...
    m_jacobian = std::make_shared<MatrixBlock<iDynSparseMatrix>>(numberOfEquations, 6);
}
...
void ForceConstraint::evaluateJacobian()
{
    ...
    for(int i = 0; i < numberOfEquationsFrictionCone; i++)
    {
        ...
        secondPointX = pointsX((i % numberOfEquationsFrictionCone) + 1);
        secondPointY = pointsY((i % numberOfEquationsFrictionCone) + 1);

        double angularCoefficients;
        angularCoefficients = (secondPointY - firstPointY) / (secondPointX - firstPointX);

        double offset;
        offset = firstPointY - angularCoefficients * firstPointY;

        int inequalityFactor = 1;
        if(angles(i) > M_PI || angles((i % numberOfEquationsFrictionCone) + 1) > M_PI)
            inequalityFactor = -1;

        //  A_ineq(i,:) = inequalityFactor.* [-angularCoefficients, 1, (-offsets*staticFrictionCoefficient), 0, 0, 0];
        m_jacobian->matrix(i, 0) = -inequalityFactor * angularCoefficients;
        m_jacobian->matrix(i, 1) = inequalityFactor;
        m_jacobian->matrix(i, 2) = -inequalityFactor * offset * m_staticFrictionCoefficient;
    }

    // Unilateral constraint and COP position
    m_jacobian->matrix(numberOfEquationsFrictionCone, 2) = -m_torsionalFrictionCoefficient;
    m_jacobian->matrix(numberOfEquationsFrictionCone + 1, 2) = -m_torsionalFrictionCoefficient;
    m_jacobian->matrix(numberOfEquationsFrictionCone + 2, 2) = -1;
    m_jacobian->matrix(numberOfEquationsFrictionCone + 3, 2) = m_footLimitX(1);
    m_jacobian->matrix(numberOfEquationsFrictionCone + 4, 2) = -m_footLimitX(2);
    m_jacobian->matrix(numberOfEquationsFrictionCone + 5, 2) = m_footLimitY(1);
    m_jacobian->matrix(numberOfEquationsFrictionCone + 6, 2) = -m_footLimitY(2);

    m_jacobian->matrix(numberOfEquationsFrictionCone + 5, 3) = -1;
    m_jacobian->matrix(numberOfEquationsFrictionCone + 6, 3) = 1;

    m_jacobian->matrix(numberOfEquationsFrictionCone + 3, 4) = 1;
    m_jacobian->matrix(numberOfEquationsFrictionCone + 4, 4) = -1;

    m_jacobian->matrix(numberOfEquationsFrictionCone, 5) = 1;
    m_jacobian->matrix(numberOfEquationsFrictionCone + 1, 5) = -1;
...
void ForceConstraint::evaluateBounds()
{
    ...
    m_lowerBound.zero();
    m_lowerBound(3 + numberOfEquationsFrictionCone) = -m_minimalNormalForce;

    m_upperBound.zero();
    m_upperBound(3 + numberOfEquationsFrictionCone) = -m_minimalNormalForce;
